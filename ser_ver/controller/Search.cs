...
namespace курсач
{
    static class Search
    {
        public static (string, Boolean) LoginInUser(List<User> users, string loginUser, string passwordUser)
        {
            string message = "";
            bool exist = false;
        ...
        public static (string, Boolean) LoginInAdmin(List<Admin> admins, string loginAdmin, string passwordAdmin)
        {
            string message = "";
            bool exist = false;
            for (int i = 0; i < admins.Count; i++)
            {
                if (admins[i].Login == loginAdmin)
                {
                    if (admins[i].Password == passwordAdmin)
                    {
                        exist = true;
                        break;
                    }
        ...
        public static User InfoForAdmin(List<User> users, int index)
        {
            int x = -1;
            for (int i = 0; i < users.Count; i++)
            {
